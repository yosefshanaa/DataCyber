...
- **UNSW-NB15** (Moustafa & Slay, 2015, UNSW Canberra) — *modern external validation* (§7). Official
  ...
  flows); 42 features; nine attack families. Both datasets are fetched by `python data/get_data.py`.
...
│   └── unsw.py               # UNSW-NB15 schema/loader for the modern-data validation
├── tests/                    # pytest suite (unit + integration)
│   ├── test_data.py  test_features.py  test_evaluate.py
│   ├── test_models.py  test_pipeline_integration.py
...
## Tests
A `pytest` suite checks the analysis building blocks — the attack-label
taxonomy and its integrity guard, the leakage-safe preprocessing (including
robustness to `KDDTest+` categories unseen in training), the imbalance-aware
metrics, and the model factory. An integration test runs the full pipeline on
the bundled data and locks in the study's central finding (the random-split →
official-test accuracy collapse) as a regression test.
```bash
pip install -r requirements.txt   # installs pytest
pytest                            # full suite
pytest -m "not slow"              # fast unit tests only (no data needed)
pytest -m slow                    # integration tests on the bundled NSL-KDD data
```
...
*External validation on the modern **UNSW-NB15** (2015) dataset (§7):* the A→B accuracy gap reproduces
(RF 0.960 → 0.871, ≈9 pp — smaller because UNSW's official split is near-IID), aggregate accuracy again
hides near-zero recall on rare families (Analysis ≈ 0.00, Worms ≈ 0.09, Backdoor ≈ 0.10, DoS ≈ 0.10),
and the anomaly-detection remedy proves **conditional** — supervised RF wins here (MCC 0.76 vs one-class
0.46/0.17) because UNSW attacks are flow-separable, whereas NSL-KDD's R2L/U2R mimic normal traffic.

## License / academic integrity
Individual coursework. NSL-KDD © Canadian Institute for Cybersecurity; UNSW-NB15 © Moustafa & Slay,
UNSW Canberra (cite: *Military Communications and Information Systems Conf.*, 2015). Reviewed
repositories belong to their respective authors and are cited for academic critique only.
